...
export function myColAuthFilter(columns, pre) {
  let authList = getNoAuthCols(pre);
  const cols = columns.filter(item => {
      if (item.dataIndex.indexOf("_dictText") !== -1) {
        let str=item.dataIndex.slice(0, item.dataIndex.indexOf("_dictText"));
        return !authList.includes(str);
